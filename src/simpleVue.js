 ...
 * @Date: 2022-09-05 19:26:31
 * @LastEditors: tears 596231290@qq.com
 * @LastEditTime: 2022-09-05 19:26:52
 * @FilePath: /css/src/simpleVue.js
 * @Description: 这是默认设置,请设置`customMade`, 打开koroFileHeader查看配置 进行设置: https://github.com/OBKoro1/koro1FileHeader/wiki/%E9%85%8D%E7%BD%AE
 */
class SimpleVue {
  // 简化版Vue实例的构造 用来接收实例的配置项
  constructor(options) {
  ...
  observe() {
    // 数据监听器 用来劫持并监听属性值相关数据的变化 如有变化则通知订阅器watcher
    for (const key in this.$data) {
      let value = this.$data[key]
      this.oWatcherObj[key] = [] // 初始化该数据的订阅器 数据和订阅器的关系是一对多
      const oWatcherObj = this.oWatcherObj[key]
      Object.defineProperty(this.$data, key, {
        // 关键方法 可以修改对象身上的默认属性值的ES5方法 下面用到的是ES中两大属性中的访问器属性,有以下四种描述符对象
  ...
  compile(el) {
    // 节点DOM解析器 用来获取和解析每一个节点及其指令 根据初始化的模板数据来创建订阅器watcher
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (node.hasAttribute('yf-on:click')) {
        // 节点中如存在该指令则执行以下操作
        const eventAttrVal = node.getAttribute('yf-on:click')
        node.addEventListener(
          'click',
          this.$methods[eventAttrVal].bind(this.$data)
        ) // 绑定获取到的指令对应的数据所触发的方法
      }
      if (node.hasAttribute('yf-if')) {
        const ifAttrVal = node.getAttribute('yf-if')
      ...
      if (node.hasAttribute('yf-model')) {
        const modelAttrVal = node.getAttribute('yf-model')
        node.addEventListener(
          'input',
          ((i) => {
            // 前方高能:此处有闭包请绕行!!! i的问题
            this.oWatcherObj[modelAttrVal].push(
              new Watcher(this, node, 'value', modelAttrVal)
            )
            return () => {
      ...
      if (node.hasAttribute('yf-text')) {
        const textAttrVal = node.getAttribute('yf-text')
        this.oWatcherObj[textAttrVal].push(
          new Watcher(this, node, 'innerText', textAttrVal)
        )
      }
    }
  }
